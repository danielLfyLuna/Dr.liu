...
export default class HomeView extends Component {
  ...
  constructor(props) {
    super(props)
    this.columns = [
      {key: 'productId', dataIndex: 'productId', title: '产品ID', fixed: 'left', width: 80},
      {key: 'serverId', dataIndex: 'serverId', title: '服务器ID', fixed: 'left', width: 80},
      {key: 'playerCount', dataIndex: 'playerCount', title: '总人数'},
      {key: 'onlineCount', dataIndex: 'onlineCount', title: '在线人数'},
      {key: 'dottingTime', dataIndex: 'dottingTime', title: '打点时间'},
      {key: 'freePhysicalMemorySize', dataIndex: 'freePhysicalMemorySize', title: '剩余物理内存(M)'},
      {key: 'totalPhysicalMemorySize', dataIndex: 'totalPhysicalMemorySize', title: '总物理内存(M)'},
      {key: 'osname', dataIndex: 'osname', title: '操作系统名字'},
      {key: 'osarch', dataIndex: 'osarch', title: '操作系统架构'},
      {key: 'osversion', dataIndex: 'osversion', title: '操作系统版本'},
      {key: 'availableProcessors', dataIndex: 'availableProcessors', title: '可用处理器数'},
      {key: 'freeProcessMemorySize', dataIndex: 'freeProcessMemorySize', title: '虚拟机剩余内存'},
      {key: 'totalProcessMemorySize', dataIndex: 'totalProcessMemorySize', title: '虚拟机可使用内存'},
      {key: 'maxProcessMemorySize', dataIndex: 'maxProcessMemorySize', title: '虚拟机最大内存'},
      {key: 'committedVirtualMemorySize', dataIndex: 'committedVirtualMemorySize', title: '已用虚拟内存'},
      {key: 'totalSwapSpaceSize', dataIndex: 'totalSwapSpaceSize', title: '总交换空间(M)'},
      {key: 'freeSwapSpaceSize', dataIndex: 'freeSwapSpaceSize', title: '剩余交换空间(M)'},
      {key: 'systemLoadAverage', dataIndex: 'systemLoadAverage', title: '系统加载平均值'},
      {key: 'systemCpuLoad', dataIndex: 'systemCpuLoad', title: '系统负载'},
      {key: 'processCpuLoad', dataIndex: 'processCpuLoad', title: 'cpu负载'},
      {key: 'processCpuTime', dataIndex: 'processCpuTime', title: 'cpu占用率'}
    ]
  }
  ...
  render() {
    ...
    let dataSource = this.onReduce(this.props.homes.list)

    return (
      <div className='HomeView'>
        <Card>
          {
            (this.props.login.admin.status === 1) ?
              <div>
                ...
                <Row>
                  <Col>
                    <Button type='primary' onClick={this.props.reloadSP}>刷新</Button>
                  </Col>
                </Row>
              </div>
            :
