...
function fetchUserForbid(value) {
  return (dispatch, getState) => {
    ...
    let url = `${SANGO2_API_HOST}/products/${value.productId}/users`
    return axios({
      method: 'GET',
      url: url,
...
function addUserForbid(value) {
  return (dispatch, getState) => {
    ...
    dispatch(requestAdd())
    let url = `${SANGO2_API_HOST}/products/${value.productId}/users/${value.userId}`
    return axios({
      method: 'POST',
      url: url,
      data: value,
...
const ACTION_HANDLERS = {
  ...
  [USERFORBID_REMOVE_RECEIVE]: (state, action) => {
    _.map(state.list, (val, idx) => {
      if (val.userId === action.payload.data.domainObject.userId) {
