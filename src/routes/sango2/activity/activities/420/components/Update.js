...
class UpdateForm extends Component {
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      data.visibleCondition = {
        ...
        params: {
          soldierId: values.visibleCondition.params.soldiers.length ? String(values.visibleCondition.params.soldiers[0]) : values.visibleCondition.params.soldierId,
          lastDays: values.visibleCondition.params.lastDays
        }
      }
  ...
  render() {
    ...
    if (goodOpt.length) {
      soldierOpt = this._soldierReduce(goodOpt, initials.conf.goodTypes.soldier)
    }
    ...
    return (
      <Form onSubmit={this.handleSubmit}>
        ...
        >
          ...
          <FormItem
            {...formItem2Layout}
            label={intl.formatMessage(message.person)}
          >
            {getFieldDecorator('visibleCondition.params.soldiers', {
              initialValue: [Number(detail.visibleCondition.params.soldierId)],
              rules: [{ type: 'array', required: true, message: '可见条件 武将!' }]
            })(
              <Cascader options={soldierOpt} showSearch disabled={check} />
            )}
          </FormItem>
          <div>&nbsp;</div>
          <FormItem
