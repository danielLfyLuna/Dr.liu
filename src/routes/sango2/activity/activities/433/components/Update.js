    ...
    const formActivityItems = activityItems.map((option, index) => {
      ...
      return (
        <div>
          ...
          <FormItem
            {...formItemLayout}
            label={`${intl.formatMessage(message.clientValue_horse)} #${index + 1}`}
            key={index + 4}
            hasFeedback
          >
            { getFieldDecorator(`activityItems[${index}].clientValue`, {
              rules: [{ required: true, message: intl.formatMessage(message.clientValue_horse) }],
          ...
          <FormItem
            {...formItemLayout}
            label={`${intl.formatMessage(message.price)} #${index + 1}`}
            key={index + 5}
          >
            {getFieldDecorator(`activityItems[${index}].coin`, {
              initialValue: option.coin,
              rules: [{ required: true, message: intl.formatMessage(message.price) }]
            })(
              <InputNumber disabled style={{ width: '100%' }} />
            )}
          </FormItem>
          ...
          >
            {getFieldDecorator(`activityItems[${index}].level`, {
              ...
              rules: [{ required: true, message: intl.formatMessage(message.gears) }]
            })(
              <InputNumber
                disabled={check}
                min={0}
