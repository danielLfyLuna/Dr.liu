...
class DetailsForm extends Component {
  ...
  constructor(props) {
    ...
    this.columns = [{
      ...
      title: intl.formatMessage(message.alliance),
    ...
    return (
      <div>
        ...
        <Card
          title={
            <div>
              {intl.formatMessage(message.ps1)}，{intl.formatMessage(message.product)}: <i style={{color: '#d50b10'}}>{productIdOpt}</i>&nbsp;&nbsp;{intl.formatMessage(message.ps2)}: <i style={{color: '#d50b10'}}>{serverIdOpt}</i>
            </div>
          }
          noHovering
        >
          ...
          <Button type='primary'>
            <Link to='/sango2/mail/allianceMail/index'>{intl.formatMessage(message.back)}</Link>
          </Button>
          <Modal
            width={1000}
            maskClosable={false}
