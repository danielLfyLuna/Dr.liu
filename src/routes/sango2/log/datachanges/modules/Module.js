...
function fetchDatachange(value = {}) {
  return (dispatch, getState) => {
    // 验证从复提交
    if (getState().datachange.fetching) {
      return
    }
    dispatch(requestDatachange())
    let url = `${SANGO2_API_HOST}/logs/datachanges`
    value.list.productId = value.products[0]
    value.list.serverId = value.products[1]
    return axios({
      method: 'GET',
      url: url,
      params: value.list,
...
function exportDatachange(value) {
  return (dispatch, getState) => {
    dispatch(requestExportDatachange())
    openNotificationWithIcon('success', '正在导出,请稍后')
    let url = `${SANGO2_API_HOST}/logs/datachanges/export/batch`
    return axios({
      method: 'POST',
      data: value.list,
      url: url,
...
function datachangeSources() {
  return (dispatch, getState) => {
    let url = `${SANGO2_API_HOST}/logs/logsource/4`
    return axios({
      method: 'GET',
      url: url,
      headers: {
