...
function fetchRank(value) {
  return (dispatch, getState) => {
    ...
    let url = `${SANGO2_API_HOST}/products/${value.path.products[0]}/servers/${value.path.products[1]}/ranks/${value.path.rankType[0]}`
...
function fetchSpecialTypes(data) {
  return (dispatch, getState) => {
    ...
    let url = `${SANGO2_API_HOST}/products/${data.path.products[0]}/servers/${data.path.products[1]}/ranks/${data.path.type}`
    axios({
      method: 'GET',
      url: url,
      headers: {
