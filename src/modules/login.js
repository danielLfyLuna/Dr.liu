...
import { fetchSP } from '../routes/Home/modules/Module'
    ...
    apiHost
      ...
      .then(result => {
        ...
        if (data.status !== 1) {
          ...
          dispatch(getLocal({
            language: localStorage.getItem('locale') ? localStorage.getItem('locale') : navigator.language,
            userId: data.userId,
            token: data.token
          }))
          dispatch(fetchSP(data))
